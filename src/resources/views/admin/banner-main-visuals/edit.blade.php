...
@section('title', __('labels.BMV001_L001').' - '.__('labels.CM001_L009'))
@section('content')
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">{{__('labels.BMV001_L001').' - '.__('labels.CM001_L009')}}</h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.banner-main-visuals.update-confirm', $data->id) }}" id="edit_banner_main_visuals_form" method="POST" enctype="multipart/form-data">
                            @csrf
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        validation('#edit_banner_main_visuals_form', {
            'title': {
                required: true,
                maxlength: 30,
            },
    ...
    <script type="text/javascript">
        if (localStorage.getItem('valImageUpdate')) {
            $('input[name=attachment_base64]').val(localStorage.getItem('valImageUpdate'));
            $('#image-preview-src').attr("src", localStorage.getItem('valImageUpdate'));
        }
        localStorage.removeItem("valImageUpdate");
        $("#btn-storage").on('click', function() {
            localStorage.setItem('valImageUpdate', $('input[name=attachment_base64]').val());
        });
    </script>
@endsection