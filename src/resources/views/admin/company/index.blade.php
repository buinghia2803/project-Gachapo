                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">{{ __('labels.COC001_L011') }}</h3>
                            </div>
                            <!-- body start -->
                            <div class="card-body">
                                <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap4">
                                    ...
                                    <form action="{{ route('admin.company.index') }}" method="GET">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-12 d-flex">
                                                ...
                                                <a href="{{ route('admin.company.create') }}" title="" class="btn btn-primary">{{ __('labels.NML001_L003') }}</a>
                                            </div>
                                        </div>
                                    </form>
                                    <!-- filter end -->
                                                ...
                                                <table id="example1"
                                                    ...
                                                    <tr>
                                                        <form action="" method="get" id="form-sort">
                                                            <th class="tb-no custom-tb-id">{{ __('labels.COC001_L010') }}<i class="fas fa-sort mt-1 float-right sort-data" desc="{{Request::get('sort_type')}}" content-value="id"></i></th>
                                                            <th class="w-400px mw-300px">{{ __('labels.COC001_L003') }}</th>
                                                            <th>{{ __('labels.COC001_L007') }}</th>
                                                            <th class="w-100px mw-100px">{{ __('labels.NML001_L007') }}<i class="fas fa-sort mt-1 float-right sort-data" desc="{{Request::get('sort_type')}}" content-value="status"></i></th>
                                                            <th class="w-150px mw-150px">{{ __('labels.CM001_L022') }}<i class="fas fa-sort mt-1 float-right sort-data" desc="{{Request::get('sort_type')}}" content-value="created_at"></i></th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        @foreach($companies as $index => $company)
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <div class="text-overflow-ellipsis custom-text row-2">{{ $company->company }}</div>
                                                                </td>
                                                                <td><div>{{ $company->email }}</div></td>
                                                                <td><div>{{ $listStatus[$company->status] ?? '' }}</div></td>
                                                                <td><span>{{ \Carbon\Carbon::parse($company->created_at)->format('Y年m月d日')}}</span></td>
                                                                <td class="tb-action">
                                                                    <div class="d-flex align-items-center">
                                                                        <a href="{{ route('admin.company.edit', $company->id) }}" class="btn btn-primary" style="margin-right: 0 !important;">
                                    ...
                                    <div class="row mt-3">
                                        ...
                                        <div class="col-sm-12 col-md-5 d-flex justify-content-end">
                                            <a href="company?status={{ BLACKLIST }}" class="btn btn-primary mr-3">{{ __('labels.COC001_L012') }}</a>
                                            <form action="{{ route('admin.company.export-file') }}" method="POST">
                                                @csrf
                                                <input type="hidden" name="status" value="{{ Request::get('status') }}">
                                                <input type="hidden" name="id" value="{{ Request::get('id') }}">
                                                <input type="hidden" name="company" value="{{ Request::get('company') }}">
                                        ...
                                        <div class="col-md-12">
                                            <div class="dataTables_paginate paging_simple_numbers float-left" id="example1_paginate">
                                                {{ $companies->appends($params)->links() }}
                                            </div>
                                        </div>
                                    </div>
                                    <!-- end pagination -->
