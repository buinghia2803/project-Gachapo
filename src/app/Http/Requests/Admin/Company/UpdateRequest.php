...
class UpdateRequest extends BaseRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'email' => 'required|unique:companies,email,' . $this->request->get('company_id') . ',id|max:255|regex:/^[\s]{0,60}[a-zA-Z0-9][a-zA-Z0-9_+-\.]{0,60}@[a-zA-Z0-9-\.]{2,}(\.[a-zA-Z0-9]{2,4}){1,2}[\s]{0,60}$/',
            'password' => 'nullable|min:8|max:32|regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,32}$/',
            'password_confirmation' => 'nullable|min:8|max:32|same:password|regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,32}$/',
